...
function SubjectDetail({ subject, onBack, userId }) {
  ...
  const handleAddCheckin = async () => {
    ...
    try {
      ...
      const newCheckinRef = checkinCollectionRef
        .collection("checkin")
        .doc(newCno.toString());
      ...
      const studentCollectionRef = await checkinCollectionRef
        .collection("students")
        .get();
  ...
  const handleDeletestudentinchekin = async (studentId) => {
    const confirmDelete = window.confirm(
      "คุณต้องการลบนักเรียนออกจากเช็คอินนี้หรือไม่?"
    );
  ...
  const handleAddQuestion = async () => {
    ...
    try {
      ...
      const snapshot = await questionCollectionRef
  ...
  const fetchquestionList = async (checkinId) => {
    try {
      ...
      const questionData = questionSnap.docs.map((doc) => ({
        id: doc.id,
        ...doc.data(),
      }));
    ...
    try {
      ...
      const scoreData = await Promise.all(
        scoreSnap.docs.map(async (doc) => {
          ...
          const studentDate =
            studentDoc.exists &&
            studentDoc.data().date instanceof firebase.firestore.Timestamp
